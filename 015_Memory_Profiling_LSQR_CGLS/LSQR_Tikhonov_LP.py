...
class LSQR_Tikhonov_LP(Algorithm):
    ...
    def __init__(self, initial=None, operator=None, data=None, alpha=None, **kwargs):
        ...
        self.track_memory("1")
        super(LSQR_Tikhonov_LP, self).__init__(**kwargs)
        self.track_memory("2")
    ...
    def set_up(self, initial, operator, data):
        ...
        self.track_memory("11", "self.regalphasq = self.regalpha*self.regalpha")
    ...
    def run(self, iterations=None, callbacks: Optional[List[Callback]]=None, verbose=1, **kwargs):
        ...
        self.track_memory("12", "callbacks = [ProgressCallback(verbose=verbose)]")
        # transform old-style callbacks into new
        callback = kwargs.get('callback', None)
        self.track_memory("13", "callback = kwargs.get('callback', None)")
        ...
        self.track_memory("14", "callbacks.append(_OldCallback(callback, verbose=verbose))")
        ...
        self.track_memory("15", "iterations = self.max_iteration")

        if self.iteration == -1 and self.update_objective_interval>0:
            iterations+=1
        self.track_memory("16", "iterations+=1")

        # call `__next__` upto `iterations` times or until `StopIteration` is raised
        self.max_iteration = self.iteration + iterations
        self.track_memory("17", "self.max_iteration = self.iteration + iterations")

        iters = (count(self.iteration) if numpy.isposinf(self.max_iteration)
                 else range(self.iteration, self.max_iteration))
        self.track_memory("18", "iters = (count(self.iteration) if numpy.isposinf(self.max_iteration) else range(self.iteration, self.max_iteration))")
        
        for _ in zip(iters, self):
            self.track_memory("19", "update(self)")
            try:
                for callback in callbacks:
                    callback(self)
                    self.track_memory("20", "callback(self)")
    ...
    def update(self):
        ...
        self.track_memory("21", "self.u = self.operator.direct(self.v) - self.alpha * self.u")
        self.beta = self.u.norm()
        self.track_memory("22", "self.beta = self.u.norm()")
        self.u = self.u/self.beta
        self.track_memory("23", "self.u = self.u/self.beta")
        
        # update v
        self.v = self.operator.adjoint(self.u) - self.beta * self.v
        self.track_memory("24", "self.v = self.operator.adjoint(self.u) - self.beta * self.v")
        self.alpha = self.v.norm()
        self.track_memory("25", "self.alpha = self.v.norm()")
        self.v = self.v/self.alpha
        self.track_memory("26", "self.v = self.v/self.alpha")

        rhobar1 = math.sqrt(self.rhobar * self.rhobar + self.regalphasq)
        self.track_memory("27", "rhobar1 = math.sqrt(self.rhobar * self.rhobar + self.regalphasq)")
        c1 = self.rhobar / rhobar1
        s1 = self.regalpha / rhobar1
        self.track_memory("28", "s1 = self.regalpha / rhobar1")
        psi = s1 * self.phibar
        self.track_memory("29", "psi = s1 * self.phibar")
        self.phibar = c1 * self.phibar
        self.track_memory("30", "self.phibar = c1 * self.phibar")

        rho = math.sqrt(rhobar1 ** 2 + self.beta ** 2)
        self.track_memory("31", "rho = math.sqrt(rhobar1 ** 2 + self.beta ** 2)")
        c = rhobar1 / rho
        s = self.beta / rho
        self.track_memory("32", "s = self.beta / rho")
        theta = s * self.alpha
        self.track_memory("33", "theta = s * self.alpha")
        self.rhobar = -c * self.alpha
        self.track_memory("34", "self.rhobar = -c * self.alpha")
        phi = c * self.phibar
        self.phibar = s * self.phibar
        tau = s * phi
        self.track_memory("34", "tau = s * phi")
        ...
        self.track_memory("End of update", "self.normr = abs(s) * self.normr")
